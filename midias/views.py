...
def index(request):
    midias = Midias.objects.filter(usuario_id=request.user.id).order_by('-id')
...
def adicionar(request):
    if request.method == 'POST':
        ...
        nova_midia = Midias(usuario_id=request.user.id, title=title, release_year=release_year,
                            director=director, description=description, created_at=created_at)
